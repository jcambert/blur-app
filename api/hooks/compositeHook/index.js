...
function beforeValidate(composite,fields,callback){
   
    var monkey= function(model,cb){
        sails.log.info(composite);
        var composites=[];
        _.forEach( fields,function(field){ sails.log.info(model[field]);composites.push(model[field]);});
        sails.log.info(composites);
        model[composite] = composites.join(':');
        sails.log.info(model[composite]);
